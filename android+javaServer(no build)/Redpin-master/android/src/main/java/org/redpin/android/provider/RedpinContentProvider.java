...
public class RedpinContentProvider extends ContentProvider {
    ...
    public Uri insertMap(ContentValues initialValues) {
        ...
        if (!values.containsKey(RedpinContract.Map.REMOTE_ID)) {
            values.put(RedpinContract.Map.REMOTE_ID, -1);
        }

        if (!values.containsKey(RedpinContract.Map.NAME)) {
            values.put(RedpinContract.Map.NAME, r
                    .getString(android.R.string.untitled));
        }

        if (!values.containsKey(RedpinContract.Map.URL)) {
    ...
    public Uri insertLocation(ContentValues values) {
        ...
        if (!values.containsKey(RedpinContract.Location.REMOTE_ID)) {
            values.put(RedpinContract.Location.REMOTE_ID, -1);
        }

        if (!values.containsKey(RedpinContract.Location.X)
                || !values.containsKey(RedpinContract.Location.Y)) {
            Log
                    .e(TAG,
                            "the x and y need to be set in order to insert a point.");
        }

        if (!values.containsKey(RedpinContract.Location.SYMBOLIC_ID)) {
